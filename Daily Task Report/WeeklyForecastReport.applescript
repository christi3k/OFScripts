...
set filePath to "/Users/christie/Dropbox/Projects/Task Reports/Forecast/" -- Where to save the resulting text file (Be sure to add the trailing "/")
...
set fileName to "Forecast-"
set fileName to fileName & short date string of startDate
...
tell application "OmniFocus"
	tell front document
		...
		repeat with a from 1 to length of theProjects
			...
			set theTasks to (every flattened task of currentProj where its completed = false and due date is greater than or equal to startDate and due date is less than or equal to endDate)
			...
			if theTasks is not equal to {} then
				...
				repeat with b from 1 to length of theTasks
					...
					set reportText to reportText & return & "[ " & short date string of dueDate & " ] " & name of currentTask
				end repeat
			end if
		end repeat
		-- If there were no tasks to write to the report
		if reportText is equal to "" then
			set reportText to "Nothing due for this week."
		end if
		
		-- Add the date to the top of the report, enter down a couple lines, and add the report text
		set reportText to "WEEKLY FORECAST" & return & date string of startDate & " to " & date string of endDate & reportText
