...
set filePath to "/Users/christie/Dropbox/Projects/Task Reports/Weekly/" -- Where to save the resulting text file (Be sure to add the trailing "/")

-- Create the new filename as Weekly-YYYYMMDD.txt
short date string of (get current date)

set todayDate to current date
set yestDate to todayDate - 1 * days
set {year:y, month:m, day:d} to yestDate
set fileName to "Weekly-"
set fileName to fileName & short date string of todayDate

-- Set the starting date of the report
set startDate to todayDate - 6 * days
set startDate's hours to 0
set startDate's minutes to 0
set startDate's seconds to 0

-- Set the ending date of the report
set endDate to todayDate
...
tell application "OmniFocus"
	tell front document
		...
		repeat with a from 1 to length of theProjects
			...
			set theTasks to (every flattened task of currentProj where its completed = true and completion date is greater than or equal to startDate and completion date is less than or equal to endDate)
			...
			if theTasks is not equal to {} then
				...
				repeat with b from 1 to length of theTasks
					...
					set reportText to reportText & return & "[ " & short date string of completedDate & " ] " & name of currentTask
				end repeat
			end if
		end repeat
		...
		set reportText to "WEEKLY REPORT" & return & date string of startDate & " to " & date string of endDate & reportText
