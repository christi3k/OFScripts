...
set filePath to "/Users/christie/Dropbox/Projects/Task Reports/Daily/" -- Where to save the resulting text file (Be sure to add the trailing "/")

-- Create the new filename as YYYYMMDD.txt
set todayDate to current date
set yestDate to todayDate - 1 * days
set {year:y, month:m, day:d} to yestDate
set fileName to "Daily-"
set fileName to fileName & short date string of yestDate
...
tell application "OmniFocus"
	tell front document
		...
		repeat with a from 1 to length of theProjects
			...
			if theTasks is not equal to {} then
				...
				repeat with b from 1 to length of theTasks
					...
					set reportText to reportText & return & "[ " & completedTime & " ] " & name of currentTask
				end repeat
			end if
		end repeat
		...
		set reportText to "DAILY TASK LOG" & return & date string of yestDate & reportText
